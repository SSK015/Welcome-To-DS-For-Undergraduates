...
def drawRes(x: int, y: int, featMat: np.ndarray, resLst: np.ndarray, k: int, acc: float, sse: float) -> None:
    ...
    for i in range(1, k + 1):
        plt.scatter(xLst[np.nonzero(resLst == i)],
                    ...
                    s=40, label="Category" + str(i))
    # plt.scatter(centres[:, x], centres[:, y], marker='*', s=200, c='black')
    plt.title("SSE = {0:.3f}, Acc = {1:.3f}".format(sse, acc))
    plt.legend(loc='best')
    # plt.show()
    plt.savefig("Scatter.png".format(x, y, k))
...
if __name__ == "__main__":
    tagLst, featMat = loadData("归一化数据.csv")
    k = 3
    # shuffle the dataset
    ...
    x = int(input())
    y = int(input())
    drawRes(x, y, featMatrix, resTagLst, k, acc, sse)